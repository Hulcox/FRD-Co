...
const DrawerItemList1 = [
  { text: "product", icon: <Inventory />, link: "product" },
  { text: "user", icon: <Person />, link: "user" },
  { text: "order", icon: <Sell />, link: "order" },
]
const DrawerItemList2 = [
  { text: "All products", icon: <ShoppingBag />, link: "products" },
  { text: "All users", icon: <Group />, link: "users" },
  { text: "All orders", icon: <Shop />, link: "orders" },
]

const DrawerBackOffice = () => {
  const router = useRouter()
  const [selected, setSelected] = useState(router.asPath.slice(1).split("/")[1])
